...
const VideoContainer = () => {
  ...
  /* 
  `/search?part=snippet&order=viewCount&publishedAfter=${publishedAfter}&publishedBefore=${date}&key=${process.env.GOOGLE_API_KEY}`
      ...
      const response = await fetch(
        BASE_URL +
          `/videos?part=snippet%2CcontentDetails%2Cstatistics&maxResults=8&chart=mostPopular&regionCode=IN&pageToken=${
            nextPageToken ?? ""
          }&videoDuration=medium&key=` + googleApiKey
          // process.env.GOOGLE_API_KEY
      );
