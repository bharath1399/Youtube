...
import { BASE_URL, googleApiKey } from "../utils/constants";
...
const VideoSuggestions = ({ videoId, videoTitle }) => {
  const searchVideoByKeyword = async (searchText) => {
    const response = await fetch(
      BASE_URL +
        `/search?part=snippet&maxResults=8&type=video&q=${searchText}&order=viewCount&videoDuration=medium&key=${googleApiKey}`
  ...
  const getSuggestedVideos = async () => {
    const response = await fetch(
      BASE_URL +
        `/search?part=snippet&relatedToVideoId=${videoId}&maxResults=15&type=video&key=${googleApiKey}`
        // ${process.env.GOOGLE_API_KEY}`
    );
    const data = await response.json();
    if (!response.ok) {
